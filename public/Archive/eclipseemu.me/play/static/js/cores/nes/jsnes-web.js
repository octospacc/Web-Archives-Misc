    ...
    this.onAnimationFrame = () => {
      if (this.calibrating) {
        ...
        if (this.calibrationCurrentFrames === this.calibrationFrames) {
          ...
          const delta = now - this.calibrationStartTime;
          const fps = 1000 / (delta / this.calibrationFrames);
...
class GamepadController {
  ...
  disableIfGamepadEnabled(callback) {
    ...
    return (playerId, buttonId) => {
      ...
      const { playerGamepadId } = self.gamepadConfig;
...
RingBuffer.prototype.peekN = function (count) {
  ...
  const end = Math.min(this._first + count, this.capacity());
  const firstHalf = this._elements.slice(this._first, end);
  if (end < this.capacity()) {
    return firstHalf;
  }
  const secondHalf = this._elements.slice(0, count - firstHalf.length);
