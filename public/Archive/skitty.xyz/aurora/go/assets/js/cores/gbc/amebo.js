...
// I (skitty) changed some source to add cheat codes and change the palette and some orher things
...
window.gb = function(file, canvas, options) {
	...
	this.loadROMBuffer = function(buffer, battery) { //battery is an optional parameter
		...
		else app.dialog.alert('Invalid ROM');
		GBObj.game = game;
		gameLoaded = true;
		if (battery != null) {
	...
	var colours = [[255, 255, 255, 255], [169, 169, 169, 255], [84, 84, 84, 255], [0, 0, 0, 255]]
	this.file = file;
	...
	loadbios.open("GET", options.rootDir+"dmg_bios.bin");
	...
	loadCGBbios.open("GET", "roms/gbc_bios.bin");
	...
	function loadBattery() {
		...
		CRAM = new Uint8Array(RAMsizes[Math.min(3, game[0x149])]);
